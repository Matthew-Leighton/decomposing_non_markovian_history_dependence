...
def get_y_intercept(x_data, y_data):
    """
    Fits a linear regression to x-y data and returns only the y-intercept.
    
    Parameters:
    x_data (array-like): The input features (independent variables)
    y_data (array-like): The target values (dependent variable)
    
    Returns:
    float: The y-intercept of the fitted line
    """
    # Reshape x_data for sklearn if it's 1D
...
for cg_level in cg_levels:
	...
	for i in np.arange(1, 48):
		if i != 7 and i != 45:
			...
			distribution_files.append(f'word_distributions_shuffled_fly'+str(i)+'_cg'+str(cg_level)+'.npz')


	data_fractions = np.array([45,30,23,15,12,8])

	inverse_data_fractions = 45/np.array([45,30,23,15,12,8])

	#number of times to do the sub-sampling
	n=10


	inf_data_list = np.zeros((n,20))

	for i in range(n):

		random_flies = distribution_files

		I_k_list = np.zeros((len(data_fractions),20))

		for j in range(len(data_fractions)):

			data_fraction = data_fractions[j]
			
			random_flies = np.random.choice(random_flies, size=data_fraction, replace=False)
			...
			for k in np.arange(1,21):
				I_k_list[j,k-1] = I_k[k]

		for k in np.arange(1,21):
			inf_data_list[i,k-1] = get_y_intercept(inverse_data_fractions, I_k_list[:,k-1])

	I_k_mean = np.mean(inf_data_list,axis=0)
	I_k_std = np.std(inf_data_list,axis=0)
...
np.savez_compressed(f'info_cg_finitedata_shuffled.npz', results=all_results)
