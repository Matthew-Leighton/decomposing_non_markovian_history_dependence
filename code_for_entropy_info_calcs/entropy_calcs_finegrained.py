...
for i in np.arange(1,48):
    if i!=7 and i!= 45:
        # Load the word distributions
        distribution_files.append('word_distributions_fly'+str(i)+'.npz')

# Get the merged distributions directly
...
#Save as NumPy compressed file (good for later loading in Python)
np.savez_compressed(
    f'entropy_and_info_finegrained_allflies.npz',
    h_k=h_k,
    I_k=I_k
)
